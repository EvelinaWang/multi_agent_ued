...
def save_and_upload_checkpoint(params, step, save_dir, run):
    """Saves model checkpoints locally and uploads them to Wandb."""
    local_path = f"{save_dir}/checkpoint_step_{step}.pkl"
    # Save the model parameters locally
    with open(local_path, "wb") as f:
        pickle.dump(params, f)
    
    # Create and upload a Wandb artifact asynchronously
    def upload_artifact():
        artifact = wandb.Artifact(f"checkpoint-step-{step}", type="checkpoint")
        artifact.add_file(local_path)
        
# Inside the training loop
if step % checkpoint_steps == 0:
    save_and_upload_checkpoint(params, step, save_dir, run)
wandb.log_artifact(artifact)
    
    upload_thread = threading.Thread(target=upload_artifact)
    upload_thread.start()
